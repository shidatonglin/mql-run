...
/*
   ...
   CustomSignal *signal_custom =new CustomSignal(Symbol(),PERIOD_H1,PERIOD_D1,signal_bar);
   ExitSignal *signal_exit = new ExitSignal(Symbol(),PERIOD_H1,PERIOD_D1,signal_bar);
   //SignalMA *signal_ma=new SignalMA(Symbol(),0,5,2,MODE_EMA,maapplied,signal_bar);
   signal_exit.Entry(false);
   signal_exit.Exit(true);
   CSignals *signals=new CSignals();
   signals.Add(GetPointer(signal_custom));
   signals.Add(GetPointer(signal_exit));
...
//|                                                                  |
//+------------------------------------------------------------------+
class CustomSignal: public CSignal
  {
protected:
   CSymbolInfo      *m_symbol;
   BreakSignal       m_signal;
   string            m_symbol_name;
   int               m_signal_bar;
   int               m_value;
   int               m_lowerTf;
   int               m_higherTf;
public:
   void              CustomSignal(const string symbol,const ENUM_TIMEFRAMES lowerTf, const ENUM_TIMEFRAMES higherTf, const int bar);
   void             ~CustomSignal();
   virtual bool      Init(CSymbolManager *symbol_man,CEventAggregator *event_man=NULL);
   virtual bool      Calculate(void);
   virtual void      Update(void);
...
//|                                                                  |
//+------------------------------------------------------------------+
void CustomSignal::CustomSignal(const string symbol,const ENUM_TIMEFRAMES lowerTf,const ENUM_TIMEFRAMES higherTf,const int bar)
  {
   m_symbol_name= symbol;
   m_signal_bar = bar;
   m_lowerTf = lowerTf;
...
//|                                                                  |
//+------------------------------------------------------------------+
void CustomSignal::~CustomSignal(void)
  {
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
bool CustomSignal::Init(CSymbolManager *symbol_man,CEventAggregator *event_man=NULL)
  {
   if(CSignal::Init(symbol_man,event_man))
     {
      if(CheckPointer(m_symbol_man))
...
//|                                                                  |
//+------------------------------------------------------------------+
bool CustomSignal::Calculate(void)
  {
   m_value = m_signal.GetAscTrendTwoTFSignal(m_lowerTf,m_higherTf,m_signal_bar);
   return true;
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
void CustomSignal::Update(void)
  {
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
bool CustomSignal::LongCondition(void)
  {
   return m_value == 1;
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
bool CustomSignal::ShortCondition(void)
  {
   return m_value == -1;
  }
...
//|                                                                  |
//+------------------------------------------------------------------+
class ExitSignal: public CSignal
  {
protected:
   CSymbolInfo      *m_symbol;
   BreakSignal       m_signal;
   ...
   int               m_higherTf;
public:
   void              ExitSignal(const string symbol,const ENUM_TIMEFRAMES lowerTf, const ENUM_TIMEFRAMES higherTf, const int bar);
   void             ~ExitSignal();
   virtual bool      Init(CSymbolManager *symbol_man,CEventAggregator *event_man=NULL);
   virtual bool      Calculate(void);
   virtual void      Update(void);
...
//|                                                                  |
//+------------------------------------------------------------------+
void ExitSignal::ExitSignal(const string symbol,const ENUM_TIMEFRAMES lowerTf,const ENUM_TIMEFRAMES higherTf,const int bar)
  {
   m_symbol_name= symbol;
   m_signal_bar = bar;
   m_lowerTf = lowerTf;
   m_higherTf = higherTf;
   m_signal.Init(m_symbol_name,PERIOD_H1);
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
void ExitSignal::~ExitSignal(void)
  {
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
bool ExitSignal::Init(CSymbolManager *symbol_man,CEventAggregator *event_man=NULL)
  {
   if(CSignal::Init(symbol_man,event_man))
     {
      if(CheckPointer(m_symbol_man))
...
//|                                                                  |
//+------------------------------------------------------------------+
bool ExitSignal::Calculate(void)
  {
   //m_value = m_signal.GetAscTrendTwoTFSignal(m_lowerTf,m_higherTf,m_signal_bar);
   m_value = m_signal.GetAscTrendSignal(m_signal_bar);
   return true;
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
void ExitSignal::Update(void)
  {
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
bool ExitSignal::LongCondition(void)
  {
   return m_value == 1;
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
bool ExitSignal::ShortCondition(void)
  {
   return m_value == -1;
  }