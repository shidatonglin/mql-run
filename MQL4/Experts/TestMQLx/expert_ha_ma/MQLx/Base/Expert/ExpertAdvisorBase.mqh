...
CExpertAdvisorBase::CExpertAdvisorBase(void) : m_active(true),
  ...
  {
  //Print("experts init-->", m_order_man.MaxOrders(), "---+++====++++", m_order_man.MaxTrades());
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
...
bool CExpertAdvisorBase::Init(string symbol,int period,int magic,bool every_tick=true,bool one_trade_per_candle=true,bool position_reverse=true)
  {
   //Print("experts init method-->maxOrders->", m_order_man.MaxOrders(), " ,-MaxTrades->", m_order_man.MaxTrades());
...
bool CExpertAdvisorBase::OnTick(void)
  {
   //Print("experts-->", m_order_man.MaxOrders(), "---+++====++++", m_order_man.MaxTrades());
   if(!Active())
      return false;
   ...
   if(IsNewBar(m_symbol_name,m_period))
      Print("orders.Total()-->", orders.Total(), " total trade-->", m_order_man.TradesTotal());
