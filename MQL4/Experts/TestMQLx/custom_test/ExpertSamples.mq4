...
input int magic1 = 18092601;
input int magic2 = 18092602;
input int magic3 = 18092603;
input int magic4 = 18092604;
...
extern int       nslow=30;
extern double    G=1.0;
extern double    dK=3.0; 
//LWMA param
extern int FasterLWMA = 7;
extern int SlowerLWMA = 30;
extern bool soundAlert = true;
extern string soundFile = "lwma_crossover.wav";
extern bool textAlert = true;
//MACD param
extern int FastEMA=30;
...
//---
   ...
   //CExpertAdvisor *expert1=expert_test_macd();
   /*
...
CExpertAdvisor * expert_test_macd()
{
   CExpertAdvisor *expert=new CExpertAdvisor();
   expert.Comment("expert_test_macd");
   expert.Init(Symbol(),Period(),magic1,true,true,true);
...
*/
   MqlParam params[1];
   SignalMACD *signal_ha=new SignalMACD(Symbol(),0,1,params,signal_bar);
   //SignalMA *signal_ma=new SignalMA(Symbol(),(ENUM_TIMEFRAMES) Period(),maperiod,0,mamethod,maapplied,signal_bar);
   CSignals *signals=new CSignals();
   signals.Add(GetPointer(signal_ha));
   //signals.Add(GetPointer(signal_ma));
...
class SignalMACD: public CSignal
  {
protected:
...
public:
   void              SignalMACD(const string symbol,const ENUM_TIMEFRAMES timeframe,const int numparams,const MqlParam &params[],const int bar);
   void             ~SignalMACD();
   virtual bool      Init(CSymbolManager *symbol_man,CEventAggregator *event_man=NULL);
   virtual bool      Calculate(void);
   virtual void      Update(void);
...
//|                                                                  |
//+------------------------------------------------------------------+
void SignalMACD::SignalMACD(const string symbol,const ENUM_TIMEFRAMES timeframe,const int numparams,const MqlParam &params[],const int bar)
  {
   m_symbol_name= symbol;
   m_signal_bar = bar;
   //m_ha=new CiHA();
...
//|                                                                  |
//+------------------------------------------------------------------+
void SignalMACD::~SignalMACD(void)
  {
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
bool SignalMACD::Init(CSymbolManager *symbol_man,CEventAggregator *event_man=NULL)
  {
   if(CSignal::Init(symbol_man,event_man))
     {
      if(CheckPointer(m_symbol_man))
...
//|                                                                  |
//+------------------------------------------------------------------+
bool SignalMACD::Calculate(void)
  {
   int bar = 1;
...
//|                                                                  |
//+------------------------------------------------------------------+
bool SignalMACD::LongCondition(void)
  {
   return (((LWMA0up != EMPTY_VALUE )||(LWMA1up != EMPTY_VALUE )) &&
                 MACDhisto>0 &&
...
//|                                                                  |
//+------------------------------------------------------------------+
bool SignalMACD::ShortCondition(void)
  {
   return (((LWMA0down != EMPTY_VALUE && LWMA0down != 0.0)||(LWMA1down != EMPTY_VALUE && LWMA1down != 0.0)) &&
                 MACDhisto<0 &&
